...
export const Navigation = ({ links, active, showModule = false, moduleLink }) => {
    ...
    <div className="navigation p-3 bg-[#1E1E2D] rounded-lg">
      <div className="bg-[#323248] rounded-lg px-[40px] py-[20px] flex items-center gap-[40px] justify-between">
        <div className="flex items-center gap-[40px]">
          ...
          {showModule && (
            <div
            onClick={moduleLink?.onClick}
            key={moduleLink?.label}
            className={`text-[14px] ${active === moduleLink?.label ? "text-[#3699FF]" : "text-[#6D6D80]"
              } uppercase cursor-pointer transition-all hover:text-[#3699FF]`}
          >
            {moduleLink?.label}
          </div>
