...
export const ProductDetail = () => {
  ...
  const { adminSession } = useSelector((state) => state.auth)
  ...
  const moduleLink = {
    ...
    onClick: () => {
      navigate(
        `/client/dashboard/products/list/module-actions/${product?.id ? product?.id : para?.id}`
      );
      setActive("MODULE ACTIONS")
    }
  }

useEffect(() => {
  if (adminSession && adminSession?.adminId && product?.moduleName) {
    setShowActions(true)
  }else{
    setShowActions(false)
  }
}, [adminSession, product])
