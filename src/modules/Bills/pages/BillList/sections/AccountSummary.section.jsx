...
const AccountSummary = () => {
  ...
  const { transactions, unpaidInvoiceCount, currentCreditBalance } =
  ...
  const columns = [
    {
      ...
      render: (date) =>
        date !== 'N/A'
          ? moment(date).format(settings?.dateFormat ?? 'DD/MM/YYYY HH:mm:ss')
          : 'N/A',
    },
    {
      title: 'DESCRIPTION',
  ...
  return (
    <div className="flex flex-col bg-custom-secondary rounded-lg p-4 md:p-[40px] flex-grow">
      <div className="flex flex-col lg:flex-row gap-[12px] mb-[20px]">
        <div className="flex flex-col md:flex-row gap-[12px] md:gap-[20px] bg-[#3A2434] rounded-lg px-[16px]">
          <div className="flex items-center justify-center">
            <span className="text-[#F64E60] font-normal text-[14px] line-[28px]">
              Unpaid Invoices
            </span>
          </div>
          <div className="flex items-center justify-center py-[16px]">
            <Badge pill bg="danger" className="text-white text-[11px] px-[6px]">
              {unpaidInvoiceCount}
            </Badge>
          </div>
        </div>
        ...
        <div className="flex flex-col md:flex-row gap-[12px] md:gap-[20px] rounded-lg px-[16px] border-1 border-[#323248] border-dashed">
          <div className="text-[14px] text-[#92928F] items-center flex py-[8px]">
            Credit Balance - $
            {currentCreditBalance ? currentCreditBalance.toFixed(2) : 0}
          </div>
          <div className="flex border-l border-[#323248] border-dashed my-[16px]" />
          <div className="text-[14px] text-[#92928F] items-center flex py-[8px]">
            Recent Invoice - $
            {recentInvoice ? recentInvoice.amount.toFixed(2) : 0}
          </div>
          <div className="flex border-l border-[#323248] border-dashed my-[16px]" />
          <div className="flex items-center justify-center">
            <Button
              ...
              onClick={() =>
                navigate(
                  `/client/dashboard/billing/download/${recentInvoice.id}`
                )
              }
              disabled={!recentInvoice}
            >
              <span className="text-[13px] leading-[20px]">Download PDF</span>
            </Button>
            <Button
              type="link"
              className="d-block py-1 text-custom-info  no-underline px-[20px] rounded hover:text-white hover:bg-black/[.2] disabled:text-[#4b4b4b] ease-in duration-100 border-0"
              onClick={() =>
                navigate(
                  `/client/dashboard/billing/detail/${recentInvoice.id}`
                )
              }
              disabled={!recentInvoice}
            >
              <span className="text-[13px] leading-[20px]">View Details</span>
            </Button>
          </div>
        </div>
          ...
          editAction={(record) => {
            return (
              <>
                ...
                <Button
                  onClick={() =>
                    navigate(`/client/dashboard/billing/detail/${record.billId}`)
                  }
                >
                  View Details
                </Button>
                <Button
                  onClick={() =>
                    navigate(
                      `/client/dashboard/billing/download/${record.billId}`
                    )
                  }
                >
                  Download PDF
      ...
      />
      <PaymentFailed show={showPaymentFailed} setShow={setShowPaymentFailed} />
      <PaymentPending
